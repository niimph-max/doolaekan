...
(async () => {
  ...
  // สมุดตัวอย่างมีบันทึกของวันก่อนๆ อยู่แล้ว แต่ "วันนี้" ต้องยังว่าง
  // ทั้งชุดทดสอบข้างล่างวัดยอดของวันนี้ ถ้าวันนี้ไม่ว่างตั้งแต่ต้น ตัวเลขจะบวก
  // กับของตัวอย่างจนไม่รู้ว่าเลขที่เห็นมาจากไหน
  const firstView = await p.locator('.screen').innerText();
  check('เห็นบันทึกของวันก่อนๆ จากสมุดตัวอย่าง', /เดินรอบหมู่บ้าน/.test(firstView));
  // อย่าเช็คคำว่า "วันนี้" จากข้อความทั้งหน้า — ปุ่ม "จดบันทึกวันนี้" ก็มีคำนั้น
  // และบรรทัดสรุปมีทุกวันที่มีบันทึก ไม่ใช่เฉพาะวันนี้
  // ตัวชี้ที่ตรงเรื่องคือหัวข้อของกลุ่มวัน ซึ่งจะเป็นคำว่า "วันนี้" ก็ต่อเมื่อ
  // วันนี้มีบันทึกแล้วจริงๆ
  const dayHeads = await p.locator('.screen h3').allInnerTexts();
  check('ยอดของวันนี้ยังไม่มี จึงเริ่มนับจากศูนย์: ' + dayHeads.join(' / '),
    !dayHeads.includes('วันนี้'));
